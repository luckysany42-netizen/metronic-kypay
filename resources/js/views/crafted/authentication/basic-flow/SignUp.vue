<template>
  <div class="w-lg-460px px-10 py-12">
    ...
    <div class="text-center mb-8">
      <h1 class="fw-bolder mb-2 text-gray-900" style="font-size: 1.6rem; letter-spacing: -0.4px;">Buat Akun Admin</h1>
      <div class="text-muted fw-semibold fs-6">
        Sudah punya akun?
        <router-link to="/sign-in" class="fw-bold ms-1" style="color: #3b82f6;">Masuk di sini</router-link>
      </div>
    </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const onSubmitRegister = async (values: any) => {
      ...
      // Gunakan adminRegister untuk registrasi admin
      await store.adminRegister(values);
      ...
      if (error.length === 0) {
        Swal.fire({
          ...
          router.push({ name: "dashboard" }); // redirect ke admin dashboard
        });
      } else {
        Swal.fire({
    ...
    return { registration, onSubmitRegister, submitButton, showPassword, showPasswordConfirm, getAssetPath };
  },
});
</script>
